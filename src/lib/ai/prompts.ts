...
export function buildDailyRoutineSystemPrompt(communityName: string, city?: string | null): string {
  return `Tu es l'Assistant IA de communication de "${communityName}"${city ? ` (${city})` : ""}.
...
export function buildSystemPrompt(community: {
  ...
  return `Tu es l'Assistant IA de communication de "${community.name}", une ${communityTypeDesc}${community.city ? ` basée à ${community.city}` : ""}${community.religiousStream ? `, de tradition ${community.religiousStream}` : ""}.
