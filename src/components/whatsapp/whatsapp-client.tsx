      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-start gap-3">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700">
            {connected ? <CheckCircle2 className="size-5" /> : <Smartphone className="size-5" />}
          </span>
          <div>
            <p className="text-sm font-semibold text-slate-950">
              {connected ? "WhatsApp connecté" : "Connecter WhatsApp avec QR code"}
            </p>
            <p className="mt-1 text-sm leading-6 text-slate-600">
              {connected
                ? "Votre numéro est prêt pour les envois WhatsApp depuis EasyCom IA."
                : "Scannez un QR code depuis WhatsApp pour envoyer avec votre numéro personnel."}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          {canDisconnect && (
            <Button
              ...
              className="h-10 rounded-2xl border-emerald-200 text-emerald-800 hover:bg-emerald-50"
            >
...
function WhatsAppPhonePreview({
  ...
  return (
    <div className="sticky top-6 mx-auto flex w-full justify-center">
...
export function WhatsAppClient({
  ...
  const [topDisconnecting, setTopDisconnecting] = useState(false);
  ...
  async function disconnectWhatsAppFromHeader() {
    setTopDisconnecting(true);
    try {
      await fetch("/api/whatsapp/qr", { method: "DELETE" });
      setConnectionState({ connected: false, canDisconnect: false });
      setConnectionPanelKey((current) => current + 1);
    } finally {
      setTopDisconnecting(false);
    }
  }
      ...
      <section className="space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <span className={cn("size-2 rounded-full", isConnected ? "bg-emerald-500" : "bg-amber-500")} />
              {isConnected ? "WhatsApp connecte" : "WhatsApp a connecter"}
            </div>
            {connectionState.canDisconnect && (
              <Button
                type="button"
                variant="outline"
                onClick={() => void disconnectWhatsAppFromHeader()}
                disabled={topDisconnecting}
                className="h-9 rounded-full border-red-200 bg-white px-3 text-xs font-semibold text-red-600 hover:bg-red-50 hover:text-red-700"
              >
                {topDisconnecting ? <Loader2 className="size-3.5 animate-spin" /> : <WifiOff className="size-3.5" />}
                Deconnecter
              </Button>
            )}
          </div>
          <div className="hidden">
            ...
            {isConnected ? "WhatsApp connecté" : "WhatsApp à connecter"}
          </div>
        </div>
      </section>

      <WhatsAppConnectionPanel
        key={connectionPanelKey}
        isPaid={isPaid}
