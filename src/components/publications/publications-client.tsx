...
const SOCIAL_LOGOS: Record<string, React.ReactNode> = {
  ALL: (
    <svg className="size-4.5 fill-[#3b82f6]" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  INSTAGRAM: (
    <svg className="size-4.5 stroke-[2] fill-none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" stroke="url(#ig-grad-pub)">
      <defs>
        <linearGradient id="ig-grad-pub" x1="0%" y1="100%" x2="100%" y2="0%">
  ...
  WEB: (
    <svg className="size-4.5 stroke-[#3b82f6] fill-none stroke-[2]" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
const CHANNEL_BRAND_CLASSES: Record<string, { bg: string; text: string; border: string }> = {
  WHATSAPP: { bg: "bg-emerald-500", text: "text-white", border: "border-emerald-400" },
  INSTAGRAM: { bg: "bg-gradient-to-br from-pink-500 via-rose-500 to-orange-400", text: "text-white", border: "border-pink-400" },
  FACEBOOK: { bg: "bg-blue-600", text: "text-white", border: "border-blue-500" },
  TELEGRAM: { bg: "bg-sky-500", text: "text-white", border: "border-sky-400" },
  EMAIL: { bg: "bg-slate-700", text: "text-white", border: "border-slate-500" },
  WEB: { bg: "bg-blue-900", text: "text-white", border: "border-blue-800" },
};
...
export function PublicationsClient({ publications, statsByStatus, activeStatus, activeChannel }: Props) {
  ...
  function renderPublicationCard(pub: Publication) {
    const brand = CHANNEL_BRAND_CLASSES[pub.channelType] ?? { bg: "bg-slate-100", text: "text-slate-600", border: "border-slate-200" };
    return (
      <Card
        key={pub.id}
        className={cn(
          "border-slate-200/90 bg-white/95 transition-shadow hover:shadow-md hover:shadow-blue-100/30 rounded-2xl overflow-hidden",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 pb-1 justify-center">
        {[
          ...
          { label: "Email", value: "EMAIL", logo: SOCIAL_LOGOS.EMAIL, colorClass: "bg-slate-800 text-white border-transparent hover:bg-slate-900" },
        ].map((tab) => {
          const isActive = currentChannel === tab.value;
          const href = tab.value === "ALL"
            ? (activeStatus ? `/dashboard/publications?status=${activeStatus}` : "/dashboard/publications")
            : `/dashboard/publications?channel=${tab.value}${activeStatus ? `&status=${activeStatus}` : ""}`;

          return (
            <Link key={tab.value} href={href}>
              <button
                className={cn(
                  "flex items-center gap-2 rounded-2xl border px-4 py-3 text-xs font-bold transition-all shadow-sm hover:scale-[1.02] active:scale-[0.98] cursor-pointer",
                  isActive
                    ? tab.colorClass
      ...
      <div className="flex flex-wrap gap-1.5 justify-center">
        {[
          ...
          const href = filter.value
            ? `/dashboard/publications?status=${filter.value}${activeChannel ? `&channel=${activeChannel}` : ""}`
            : (activeChannel ? `/dashboard/publications?channel=${activeChannel}` : "/dashboard/publications");

          return (
            <Link key={filter.value} href={href}>
              <button
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold transition-all duration-200 hover:-translate-y-0.5 active:scale-[0.98] cursor-pointer",
                  isActive
                    ? "bg-[#0d2f6b] text-white shadow-sm"
                    : "border border-slate-200 bg-white text-slate-600 hover:bg-blue-50/50 hover:shadow-sm"
                )}
              >
                {filter.label}
      ...
      {(statsByStatus["FAILED"] ?? 0) > 0 && !activeStatus && (
        <div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-4 max-w-4xl mx-auto">
        ...
        {publications.length === 0 ? (
          <div className="flex min-h-64 flex-col items-center justify-center rounded-[32px] border border-dashed border-slate-200 bg-slate-50/70 px-6 py-12 text-center">
            ...
            <p className="mt-1 text-xs text-slate-500 max-w-sm">
              {activeStatus
                ? "Aucune publication ne correspond à ce filtre pour ce statut."
                : "Vous n'avez pas encore de publications sur ce canal."}
            </p>
          </div>
        ) : (
