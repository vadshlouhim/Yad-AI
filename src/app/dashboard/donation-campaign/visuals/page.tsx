    ...
    if (dueSteps.length > 0) {
      ...
      await Promise.allSettled(
        dueSteps.map(async (step) => {
          ...
          await Promise.allSettled([
            admin2.from("Notification").insert({
              id: crypto.randomUUID(),
              userId: profile.id,
              communityId: profile.communityId,
              type: "EVENT_REMINDER",
              title: `⏰ Il est l'heure — ${step.step_label}`,
              body: `L'étape "${step.step_label}" était prévue à ${scheduledDate}. Il est temps d'envoyer votre message !`,
              link: "/dashboard/donation-campaign/visuals",
              data: { stepId: step.id, campaignId: campaign.id },
            }),
            notifyUser(admin2, profile.id, {
              title: `⏰ Il est l'heure — ${step.step_label}`,
