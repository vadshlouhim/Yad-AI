...
import { listGmbReviews } from '@/lib/gmb/reviews';
...
export async function GET(request: Request) {
  ...
  const result = await listGmbReviews(admin, profile.communityId, url);
  if ('error' in result) {
    const status = result.error.includes('pas connecté') ? 400 : 500;
