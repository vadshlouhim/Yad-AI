...
export async function POST(request: Request) {
  try {
    ...
    const { data: facebookChannel } = await admin
      .from("Channel")
      .select("id")
      .eq("communityId", communityId)
      .eq("type", "FACEBOOK")
    ...
    if (!facebookChannel?.id) {
      return NextResponse.json({ error: "Aucun canal Facebook connecte" }, { status: 400 });
    }
    ...
    await admin.from("ContentDraft").insert({
      id: draftId,
      communityId,
      title: cleanContent.slice(0, 120) || "Publication Facebook",
      body: cleanContent,
      hashtags,
      ...
      aiGenerated: false,
      aiPromptUsed: "facebook-manual-page",
      updatedAt: now,
    });
    ...
    await admin.from("ChannelAdaptation").upsert(
      {
        ...
        hashtags,
        imageUrl: primaryMediaUrl,
        metadata: { mediaUrls, source: "facebook-manual-page" },
        updatedAt: now,
      },
